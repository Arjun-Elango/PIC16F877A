...
/*problem statement PULL DOWN CONCEPT INTERFACING USER PUSHBUTTON FOR CONTROL LED
         |  RD3   |  RD5   |
 * RC0=0 | Off(0) |  ON(1) |
 * RC1=0 | ON(1)  |  OFF(0)|
 * RC2=0 | ON(1)  |  ON(1) |
...
void main(void){
    ...
    while(1){     // loop , continous runing the block of code
        status = PORTC;
        switch(status){
            case 0x01: // 0000 0001 if RC0 is pressed then RD5 should turn on
                PORTD= 0x20; //0010 0000  it high the RD5
                break;
            case 0x02: // 0000 0010 if RC1 is pressed then RD3 should turn on
                PORTD= 0x08; //0000 1000 it high the RD3
                break;
            case 0x04: // 0000 0100 if RC2 is pressed then both should be turn on
                PORTD= 0x28; //0010 1000 it high the RD5 & RD3
                break;
            case 0x08: // 0000 1000 if RC3 is pressed then both should be turn off
                PORTD= 0x00; // 0000 0000  it low the LED 
                break;
            case 0x00: // IF none of the switch is pressed both led should be low
                PORTD= 0x00; // 0000 0000 it low the LED
                break;
        }
